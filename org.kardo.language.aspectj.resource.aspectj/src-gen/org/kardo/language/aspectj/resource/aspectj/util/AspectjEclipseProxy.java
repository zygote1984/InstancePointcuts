 ...
 */
public class AspectjEclipseProxy {
	...
	public void getDefaultLoadOptionProviderExtensions(java.util.Map<Object, Object> optionsMap) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.kardo.language.aspectj.resource.aspectj.mopp.AspectjPlugin.EP_DEFAULT_LOAD_OPTIONS_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					org.kardo.language.aspectj.resource.aspectj.IAspectjOptionProvider provider = (org.kardo.language.aspectj.resource.aspectj.IAspectjOptionProvider) element.createExecutableExtension("class");
					final java.util.Map<?, ?> options = provider.getOptions();
					final java.util.Collection<?> keys = options.keySet();
					for (Object key : keys) {
						org.kardo.language.aspectj.resource.aspectj.util.AspectjMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	...
	public void getResourceFactoryExtensions(java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			org.eclipse.core.runtime.IExtensionRegistry extensionRegistry = org.eclipse.core.runtime.Platform.getExtensionRegistry();
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.kardo.language.aspectj.resource.aspectj.mopp.AspectjPlugin.EP_ADDITIONAL_EXTENSION_PARSER_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
	 ...
	 */
	public org.kardo.language.aspectj.resource.aspectj.mopp.AspectjResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(org.kardo.language.aspectj.resource.aspectj.IAspectjTextResource resource, org.eclipse.emf.ecore.EObject root, boolean includeBatchConstraints) {
		...
		org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil runtimeUtil = new org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil();
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					if (includeBatchConstraints) {
						...
						addStatus(status, resource, root, org.kardo.language.aspectj.resource.aspectj.AspectjEProblemType.BATCH_CONSTRAINT_PROBLEM);
					}
					...
					addStatus(status, resource, root, org.kardo.language.aspectj.resource.aspectj.AspectjEProblemType.LIVE_CONSTRAINT_PROBLEM);
				} catch (Throwable t) {
					new org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	...
	public void addStatus(org.eclipse.core.runtime.IStatus status, org.kardo.language.aspectj.resource.aspectj.IAspectjTextResource resource, org.eclipse.emf.ecore.EObject root, org.kardo.language.aspectj.resource.aspectj.AspectjEProblemType problemType) {
	...
	public String getPlatformResourceEncoding(org.eclipse.emf.common.util.URI uri) {
		...
		if (!new org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil().isEclipsePlatformRunning()) {
			return null;
		}
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new org.kardo.language.aspectj.resource.aspectj.util.AspectjRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
