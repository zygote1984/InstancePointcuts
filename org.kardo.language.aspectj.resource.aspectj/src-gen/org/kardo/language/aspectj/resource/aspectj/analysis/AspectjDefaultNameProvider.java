...
public class AspectjDefaultNameProvider implements org.kardo.language.aspectj.resource.aspectj.IAspectjNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.kardo.language.aspectj.resource.aspectj.util.AspectjEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
