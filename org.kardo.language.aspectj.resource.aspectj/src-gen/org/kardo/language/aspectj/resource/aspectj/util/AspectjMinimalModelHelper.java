 ...
 */
public class AspectjMinimalModelHelper {
	
	private final static org.kardo.language.aspectj.resource.aspectj.util.AspectjEClassUtil eClassUtil = new org.kardo.language.aspectj.resource.aspectj.util.AspectjEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.kardo.language.aspectj.resource.aspectj.util.AspectjStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.kardo.language.aspectj.resource.aspectj.util.AspectjListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.kardo.language.aspectj.resource.aspectj.util.AspectjStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.kardo.language.aspectj.resource.aspectj.util.AspectjListUtil.castListUnchecked(value);
