...
/**
 * The AspectjTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * AspectjTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class AspectjTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.kardo.language.aspectj.resource.aspectj.mopp.AspectjTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.kardo.language.aspectj.resource.aspectj.mopp.AspectjTaskItem> foundItems = new java.util.ArrayList<org.kardo.language.aspectj.resource.aspectj.mopp.AspectjTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.kardo.language.aspectj.resource.aspectj.mopp.AspectjTaskItem(keyword, message, localLine, offset, end));
