...
/**
 * The IpcTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * IpcTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class IpcTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.kardo.language.ipc.resource.ipc.mopp.IpcTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.kardo.language.ipc.resource.ipc.mopp.IpcTaskItem> foundItems = new java.util.ArrayList<org.kardo.language.ipc.resource.ipc.mopp.IpcTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.kardo.language.ipc.resource.ipc.mopp.IpcTaskItem(keyword, message, localLine, offset, end));
